...
SUBROUTINE tea_leaf_kernel_ppcg_inner(x_min,             &
  ...
  INTEGER(KIND=4) :: j,k,l, step, dev_id
  REAL(KIND=8), DIMENSION(:) :: alphas, betas
  REAL(KIND=8) :: smvp, rx, ry, rz, alpha, beta
...
!$OMP DO
  DO l=z_min,z_max
    DO k=y_min,y_max
        DO j=x_min,x_max
            sd(j, k, l) = alpha*sd(j, k, l) + beta*r(j, k, l)
        ENDDO
    ENDDO
  ENDDO
!$OMP END DO
