...
CONTAINS
    ...
    SUBROUTINE tea_pack_message_left(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        INTEGER      :: j,k,l,x_inc,y_inc,z_inc,index,buffer_offset,offload,dev_id
        ...
        !$OMP MAP(TO:depth) MAP(FROM:left_snd_buffer)
    ...
    SUBROUTINE tea_unpack_message_left(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
        !$OMP MAP(TO:depth) MAP(TO:left_rcv_buffer) 
        !$OMP PARALLEL DO PRIVATE(index)
        DO l=z_min-depth,z_max+z_inc+depth
            DO k=y_min-depth,y_max+y_inc+depth
                DO j=1,depth
    ...
    SUBROUTINE tea_pack_message_right(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
        !$OMP MAP(TO:depth) MAP(FROM:right_snd_buffer) 
        !$OMP PARALLEL DO PRIVATE(index) 
        DO l=z_min-depth,z_max+z_inc+depth
            DO k=y_min-depth,y_max+y_inc+depth
                DO j=1,depth
    ...
    SUBROUTINE tea_unpack_message_right(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
        !$OMP MAP(TO:depth) MAP(TO:right_rcv_buffer) 
        !$OMP PARALLEL DO PRIVATE(index) 
        DO l=z_min-depth,z_max+z_inc+depth
            DO k=y_min-depth,y_max+y_inc+depth
                DO j=1,depth
    ...
    SUBROUTINE tea_pack_message_top(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        DO k=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(FROM:top_snd_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO l=z_min-depth,z_max+z_inc+depth
    ...
    SUBROUTINE tea_unpack_message_top(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        DO k=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(TO:top_rcv_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO l=z_min-depth,z_max+z_inc+depth
                DO j=x_min-depth,x_max+x_inc+depth
                    index= buffer_offset + (j+depth) + &
                        (l-1+depth)*(x_max+x_inc+2*depth) + &
                        (k-1)*((x_max+x_inc+2*depth)*(z_max+z_inc+2*depth))
    ...
    SUBROUTINE tea_pack_message_bottom(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        DO k=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(FROM:bottom_snd_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO l=z_min-depth,z_max+z_inc+depth
                DO j=x_min-depth,x_max+x_inc+depth
                    index= buffer_offset + (j+depth) + &
    ...
    SUBROUTINE tea_unpack_message_bottom(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        DO k=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(TO:bottom_rcv_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO l=z_min-depth,z_max+z_inc+depth
                DO j=x_min-depth,x_max+x_inc+depth
                    index= buffer_offset + (j+depth) + &
    ...
    SUBROUTINE tea_pack_message_back(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        DO l=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(FROM:back_snd_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO k=y_min-depth,y_max+y_inc+depth
                DO j=x_min-depth,x_max+x_inc+depth
                    index= buffer_offset + &
    ...
    SUBROUTINE tea_unpack_message_back(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        DO l=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(TO:back_rcv_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO k=y_min-depth,y_max+y_inc+depth
                DO j=x_min-depth,x_max+x_inc+depth
                    index= buffer_offset + (j+depth) + &
    ...
    SUBROUTINE tea_pack_message_front(x_min,x_max,y_min,y_max,z_min,z_max,field,                 &
        ...
        DO l=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(FROM:front_snd_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO k=y_min-depth,y_max+y_inc+depth
                DO j=x_min-depth,x_max+x_inc+depth
                    index= buffer_offset + (j+depth) + &
    ...
    SUBROUTINE tea_unpack_message_front(x_min,x_max,y_min,y_max,z_min,z_max,field,                &
        ...
        DO l=1,depth
            !$OMP TARGET IF(offload.EQ.1) DEVICE(dev_id) &
            !$OMP MAP(TO:depth) MAP(TO:front_rcv_buffer) 
            !$OMP PARALLEL DO PRIVATE(index) 
            DO k=y_min-depth,y_max+y_inc+depth
                DO j=x_min-depth,x_max+x_inc+depth
                    index= buffer_offset + (j+depth) + &
