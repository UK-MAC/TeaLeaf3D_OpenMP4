...
!$OMP TARGET DEVICE(dev_id) MAP(TO:ch_alphas,ch_betas)
!$OMP PARALLEL 
...
SUBROUTINE tea_leaf_kernel_cheby_iterate(x_min,             &
  ...
  INTEGER :: j,k,l, dev_id
    ...
    REAL(KIND=8) ::  rx, ry, rz, alpha, beta

    INTEGER :: step, max_cheby_iters
    REAL(KIND=8), DIMENSION(max_cheby_iters) :: ch_alphas, ch_betas

    alpha = ch_alphas(step)
    beta = ch_betas(step)
  ...
  DO l=z_min,z_max
    DO k=y_min,y_max
        DO j=x_min,x_max
            ...
            p(j, k, l) = alpha*p(j, k, l) + beta*Mi(j, k, l)*r(j, k, l)
        ENDDO
    ENDDO
  ENDDO
!$OMP END DO
  ...
  DO l=z_min,z_max
    DO k=y_min,y_max
        DO j=x_min,x_max
            ...
            p(j, k, l) = alpha*p(j, k, l) + beta*r(j, k, l)
        ENDDO
    ENDDO
  ENDDO
!$OMP END DO
