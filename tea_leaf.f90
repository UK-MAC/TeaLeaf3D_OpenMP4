...
!$OMP PARALLEL
    IF(parallel%boss)THEN
        IF(OMP_GET_THREAD_NUM().EQ.0) THEN
            ...
            WRITE(*,'(a33)') 'TeaLeaf MPI + OpenMP 4.0 Version'
            WRITE(*,'(a13,i7)') 'Task Count: ', parallel%max_task 
            WRITE(*,'(a15,i5)') 'Thread Count: ', OMP_GET_NUM_THREADS()
            WRITE(*,*)
            WRITE(0,*)
            !WRITE(0,'(a15,f8.3)') 'Tea Version ',g_version
            WRITE(0,'(a33)') 'TeaLeaf MPI + OpenMP 4.0 Version'
            WRITE(0,'(a13,i7)') 'Task Count: ', parallel%max_task
